...
class InMemoryIOHandler implements io.IOHandler {
  ...
  async load(): Promise<io.ModelArtifacts> {
    ...
    return this.getModelArtifactsForJSON(this.modelJSON, (weightsManifest) =>
...
export class ModelOperations {
  ...
  constructor(modelOptions: ModelOperationsOptions) {
    ...
    this._minContentSize =
      modelOptions?.minContentSize ?? ModelOperations.DEFAULT_MIN_CONTENT_SIZE;
    this._maxContentSize =
      modelOptions?.maxContentSize ?? ModelOperations.DEFAULT_MAX_CONTENT_SIZE;
  ...
  private async loadModel() {
    ...
    if (!isBackendRegistered) {
      ...
      if (!(await setBackend('webgl'))) {
        throw new Error('Unable to set backend to WEBGL.');
      }
